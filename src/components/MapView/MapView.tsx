...
const MapView : React.FunctionComponent = () => {
    ...
    const showTrail = useCallback((trailId: number) => {
        setTrailVisibility(previousState => ({...previousState, [trailId]: true}))
    }, []
    )
    const hideTrail = useCallback((trailId: number) => {
        setTrailVisibility(previousState => ({...previousState, [trailId]: false}))
    }, []
    )
    ...
    return (
        <>
            {trails ? (
                <>
                    <TrailMap trails={trails} showTrail={showTrail} hideTrail={hideTrail}/>
